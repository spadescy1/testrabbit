...
const addCart = () => {
  if (skuObj.skuId) {
    ...
    cartStore.addCart({
      ...
      price: goods.value.price, // 最所价格
      count: count.value, // 商品数
      skuId: skuObj.skuId, // skuId
      attrsText: skuObj.specsText, // 商品规格文本
      selected: true, // 两品是否选中
    });
  } else {
    //规格没有选择 提示用户
    ...
    <div class="container" v-if="goods.details">
      <div class="bread-container">
        <el-breadcrumb separator=">">
          ...
          <!-- 
            会报undefined错,错误原因：
                goods一开始{}是空对象 ————>{}.categories,去点空对象的categories，就没有东西 
                ————> 出现undefined ————> undefined[1]就没有东西 
            解决方式：
                1.可选链的语法———— ?. 
                2.v-if      手动控制渲染时机 保证只有数据存在才渲染 
            
            ?.  意思是，如果前面的categories有值，它才往后继续计算
